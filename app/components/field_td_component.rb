...
class FieldTdComponent < ViewComponent::Base
  ...
  def field_tag
    ...
    when "integer"
      number_field_tag field_name, @value, opts.merge(class: "layer-table__td--numerical", step: "1")
    when "float"
      number_field_tag field_name, @value, opts.merge(class: "layer-table__td--numerical", step: "any")
    when "territory"
  ...
  def html_class
    class_names("table-field", dom_id(@field), ("layer-table__td--boolean" if @field.type_boolean?))
  end
